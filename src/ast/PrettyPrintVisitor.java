...
public class PrettyPrintVisitor implements Visitor
{
	...
	private void createFile(String fullyQualifiedName) throws IOException
	{
		...
		this.filePath = "smalioutput/"
						+fullyQualifiedName.substring(1,fullyQualifiedName.length()-1) 
						+ ".smali";
		...
		else
		{
			for(ast.stm.T stm : method.statements)
			{
				if( 	labelIndex < method.labelList.size()
						...
						|| currentLab.lab.startsWith("pswitch_data")
						|| currentLab.lab.startsWith("array_"))
						&&(stm instanceof ast.stm.Instruction.PackedSwitchDirective 
						|| stm instanceof ast.stm.Instruction.SparseSwitchDirective
						|| stm instanceof ast.stm.Instruction.ArrayDataDirective)   )
				{
					this.printSpace();
